...
class Indicator():
    ...
    def __init__(self):
        ...
        selected_device = 'battery_hidpp_battery_'
        device_icon = ''
        ...
        lp = 100
        perc = '??'
        args1 = ['/bin/sh','-c','upower -i /org/freedesktop/UPower/devices/battery_hidpp_battery_0 | grep model']
        try:
            s = subprocess.check_output (args1).decode ('Utf-8').split()  # check for specific keyboard model
        except subprocess.CalledProcessError:
            s = ''
        di = 0 if s[1] == 'K800' else 1
        try:
        ...
        for opt, arg in opts:
            if opt == '-k':
                selected_device += str (di)
                device_icon = 'kbd'
            elif opt == '-m':
                selected_device += str (1 - di)
                device_icon = 'mse'
            elif opt == '-w':
                selected_theme = 'w'
            elif opt == '-p':
        ...
        self.indicator = AppIndicator3.Indicator.new (
            ...
            os.path.join (os.path.abspath (os.path.dirname (sys.argv[0])),
                          icon_folder+"/bat_"+device_icon+"_"+selected_theme+"_nn.png"),
                          AppIndicator3.IndicatorCategory.HARDWARE)
        self.indicator.set_status (AppIndicator3.IndicatorStatus.ACTIVE)
        self.indicator.set_menu (self.create_menu())
        if show_percentage: self.indicator.set_label ("? %", self.app)
    ...
    def show_perc (self):
        ...
        while True:
            ...
            GObject.idle_add(self.indicator.set_icon, 
                os.path.join (os.path.abspath (os.path.dirname (sys.argv[0])),
                              icon_folder + "/bat_" + device_icon + "_" + selected_theme + m + "_" + iname + ".png"),
    ...
    def about (self, source):
        ...
        about_dialog.set_logo (GdkPixbuf.Pixbuf.new_from_file (os.path.join(os.path.abspath (os.path.dirname (sys.argv[0])),
                                                                            icon_folder + "/battery-charge-100.png")))
        authors = ["GNOME Documentation Team"]
        documenters = ["GNOME Documentation Team"]
        about_dialog.set_program_name (self.app)
        about_dialog.set_copyright ("Copyright \xa9 7527 \u0412\u043B\u0430\u0434\u0438\u043C\u0456\u0440\u044A")
    ...
    def message (self, source):
        ...
        messagedialog = Gtk.MessageDialog (type = Gtk.MessageType.INFO, buttons = Gtk.ButtonsType.OK, message_format = mf)
